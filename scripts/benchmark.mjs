...
for (let i = 0; i < versionsToCompare.length; i++) {
  ...
  for (let j = 0; j < reps; j++) {
    ...
    console.log(
      `${progressString} (${
        jobIndex + 1
      }/${totalJobs}) Benchmarking git-truck@${version}...`
    );
    console.log();
    cleanUp();
    // Mark time
    ...
    invariant(
      output
        .toString()
        .includes(`https://github.com/git-truck/git-truck#readme`),
  ...
  const globalInstallAverage =
    globalInstallTimes.reduce((a, b) => a + b, 0) / globalInstallTimes.length;
