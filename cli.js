...
const importJSON = async (path) => (await import(path, { assert: { type: "json" } })).default

;(async () => {
  const fs = await importDefault("fs")
  const { exec, execSync } = await import("child_process")
  const yargsParser = await importDefault("yargs-parser")
  const pkg = await importJSON("./package.json")
  const { compare, valid, clean } = await importDefault("semver")
  const open = await importDefault("open")
  const args = parseArgs()

  const currentV = pkg.version
  let updateMessage = ""
  const latestVersion = await importDefault("latest-version")
  try {
