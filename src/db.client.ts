...
import { CommitDTO, DBFileChange, GitLogEntry, RawGitObject, RenameEntry, RenameInterval } from "./analyzer/model"
import { Inserter } from "./analyzer/DBInserter"
import { getTimeIntervals } from "./analyzer/util"
...
export class DB {
  ...
  private constructor(
    ...
    this.selectedRange = [0, 1_000_000_000_000]
  }

  /**
   * Execute a function with a connection to the database
   * @param fn
   * @returns
   */
  async with<T>(fn: (connection: duckdb.AsyncDuckDBConnection) => Promise<T>) {
  ...
  static async createInstance(repo: string, branch: string) {
    ...
    const db = new DB(duckDbInstance, connection, repo, branch)

    await db.with(async (c) => {
      await DB.initTables(c)
      await DB.initViews(c, 0, 1_000_000_000_000)
    })

    return db
  }

  private static async initTables(connection: duckdb.AsyncDuckDBConnection) {
  ...
  private static async initViews(connection: duckdb.AsyncDuckDBConnection, start: number, end: number) {
    await connection.query(`
