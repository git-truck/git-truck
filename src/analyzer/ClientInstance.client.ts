...
import { log } from "./log"
import { GitCaller } from "./git-caller.client"
...
export default class ClientInstance {
  ...
  public selectedRange: [number, number] = [0, 0]
  ...
  private constructor(
    ...
    public path: string,
    public db: DB,
    public gitCaller = new GitCaller(repo, branch, path),
    private repoSanitized = repo.replace(/\W/g, "_"),
    private branchSanitized = branch.replace(/\W/g, "_")
  ) {
  }

  public static async createInstance(repo: string, branch: string, path: string) {
    const db = await DB.createInstance(repo, branch)
    return new ClientInstance(repo, branch, path, db)
  }
  ...
  private flattenChains(chains: RenameInterval[][]) {
    return chains.flatMap<RenameInterval>((chain) => {
      const destinationName = chain[0].toname
      return chain.map((interval) => ({ ...interval, toname: destinationName }))
    })
  }
