...
export const gitLogRegex =
  /"author\s+<\|(?<author>.*?)\|>\s+date\s+<\|(?<datecommitter>\d+)\s(?<dateauthor>\d+)\|>\s+message\s+<\|(?<message>[\s\S]*?)\|>\s+body\s+<\|(?<body>[\s\S]*?)\|>\s+hash\s+<\|(?<hash>.+?)\|>"\s*(?<contributions>(?:(?:\d+|-)\s+(?:\d+|-)\s+.+\s?)*)(?<modes>(?:\s.+\s*)*)/ugm
